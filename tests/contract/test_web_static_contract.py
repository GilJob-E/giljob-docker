...
class WebStaticContractTest(unittest.TestCase):
    ...
    def test_app_js_uses_browser_public_url_and_hides_tokens_from_log(self) -> None:
        ...
        self.assertIn("/stt/warmup", body)
        self.assertIn("warmSttModel", body)
        self.assertIn("void warmSttModel();", body)
        self.assertIn('params.set("partial", "true")', body)
        self.assertIn("PARTIAL_TRANSCRIPTION_INTERVAL_MS", body)
        self.assertIn("AbortController", body)
        self.assertIn("실시간 전사", body)
        self.assertIn("최종 전사", body)
