...
- Keep legacy browser-facing question and TTS routes fail-closed with `deprecated_ai_engine_removed` / `realtime_only`; avatar-session metadata is API-owned and disabled/deferred unless separately verified. Never expose provider keys, raw session tokens, or upstream error bodies.
