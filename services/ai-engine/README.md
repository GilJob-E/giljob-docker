...
The experimental Realtime-audio browser bridge is API/web-owned, not ai-engine-owned. `SPATIALREAL_BROWSER_AUDIO_BRIDGE_ENABLED` is surfaced by `services/api` as safe sibling metadata (`realtimeAvatarBridge` on `/api/sessions`, avatar-session `bridge` metadata) for the browser `AvatarPlayer.publishAudio(track)` probe; this service continues to provide only post-TTS compatibility egress and must not log or handle Realtime SDP, client secrets, raw tracks, transcripts, or provider keys for that probe.
